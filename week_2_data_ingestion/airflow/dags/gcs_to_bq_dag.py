...
with DAG(
    ...
    for colour, ds_col in COLOUR_RANGE.items():
        move_files_gcs_task = GCSToGCSOperator( # organize GCS files
            task_id=f'move_{colour}_{DATASET}_files_task',
            source_bucket=BUCKET,
            source_object=f'{INPUT_PART}/{colour}_{DATASET}*.{INPUT_FILETYPE}',
            destination_bucket=BUCKET,f
            destination_object=f'{colour}/{colour}_{DATASET}*.{INPUT_FILETYPE}', #yellow/yellow_tripdata_*.parquet
            move_object=True
        )
        ...
        bq_create_partitioned_table_job = BigQueryInsertJobOperator( # run SQL queries
